...
function RenderedMessage({ content, onOpenPanel, isStreaming = false }) {
  // Pehle yaha streaming ke dauran parsing bilkul skip ho jaati thi (raw
  // <article>/<svg> tags seedha dikhte the) kyunki poore growing string ko
  // HAR chunk pe re-parse karna O(n²) tha aur UI freeze kar deta tha.
  // Fix: parsing ab streaming mein bhi chalti hai, bas throttled — poora
  // parser sirf ~110ms mein ek baar chalta hai (max ~9x/sec), na ki har
  // chunk pe. Isse artifact/table/heading turant proper-styled dikhte hain
  // streaming ke dauran bhi, aur koi extra network/server load nahi hai —
  // yeh pura client-side render hai.
  const normalizedContent = useMemo(() => normalizeMessageMarkup(content), [content]);
  const throttledContent = useThrottledValue(normalizedContent, isStreaming ? 110 : 0);
  const blocks = useMemo(
    () => parseContent(throttledContent, { deferDataUriDecode: isStreaming }),
    ...
    if (b.type === "artifact") {
      rendered.push(
        <ArtifactCard
          ...
          isStreaming={isStreaming && !b.complete}
        />
      );
    } else if (b.type === "code") {
